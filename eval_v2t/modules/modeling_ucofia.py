...
# from modules.until_module import PreTrainedModel, AllGather, CrossEn, sinkhorn_knopp
...
class UCoFiA(CLIP4ClipPreTrainedModel):
    ...
    def get_train_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        if shaped is False:
            video_mask = video_mask.view(-1, video_mask.shape[-1])
            video = torch.as_tensor(video).float()
            b, pair, bs, ts, channel, h, w = video.shape
        ...
        return visual_output , visual_pixel_output
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, visual_pixel_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        #design 2 - seperate attention normalized + dual local mat
