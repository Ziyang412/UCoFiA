...
class UCoFiA(CLIP4ClipPreTrainedModel):
    ...
    def _loose_similarity(self, sequence_output, seq_features, visual_output, visual_pixel_output, attention_mask, video_mask, sim_header="meanP"):
        ...
        if self.training:
            ...
            visual_pixel_output = allgather(visual_pixel_output, self.task_config)
        ...
        fine_grained_sim_scores = torch.matmul(torch.matmul(word_features.view(-1, dim_text), self.local_mat_weight), torch.matmul(visual_pixel_output.view(-1, dim_video),self.local_mat_weight1).t()).view(bs_text, num_words, bs_video, num_pixels)  # [bs_text, num_words, bs_video, num_frames]
    ...
    def get_similarity_logits(self, sequence_output, seq_features, visual_output, visual_pixel_output, attention_mask, video_mask, shaped=False, loose_type=False):
        ...
        if loose_type:
            ...
            retrieve_logits = self._loose_similarity(sequence_output, seq_features, visual_output, visual_pixel_output, attention_mask, video_mask, sim_header=self.sim_header)
        else:
